...
const aprovados = [];
alumns.forEach((estudiantes) => {
  if (estudiantes.T1 + estudiantes.T2 + estudiantes.T3 >= 2) {
    estudiantes.isApproved = true;
    aprovados.push(estudiantes);
  } else {
    estudiantes.isApproved = false;
  }
});

const finales = aprovados;
console.log("🚀 ~ finales:", finales);

//? forma anterior con for:

const noSuspendidos = [];
for (let notas of alumns) {
  if (notas.T1 + notas.T2 + notas.T3 >= 2) {
    //como true es 1 y false 0, podemos hacer esta suma para comparar
    notas.isApproved = true;
    noSuspendidos.push(notas);
  } else {
    notas.isApproved = false;
  }
}

console.log(noSuspendidos);
...
placesToTravel.forEach((lugares) => {
...
//? con filter y forEach(para poder sacar los .name)

const nuevosDestinos = sitiosViaje.filter((destinos) => {
  return !(destinos.id === 11 || !destinos.id === 40);
});

nuevosDestinos.forEach((lugares) => {
  console.log("🚀 ~ lugares:", `Estos son los nuevos lugares: ${lugares.name}`);
  lugares;
});
...
//? con filter y forEach

const sinGato = toys.filter((personajes) => {
  return !personajes.name.includes("gato");
});
sinGato.forEach((personajes) => {
  console.log(
    "🚀 ~ personajes:",
    `Estos personajes no tienen la palabra gato ${personajes.name}`
  );
  return;
});
...
for (let ventas of juguetes) {
  ventas["sellCount"] > 15 ? popularToys.push(ventas["name"]) : null;
}

console.log(popularToys);

//? con filter y for each

const losMasVendidos = juguetes.filter((monecos) => {
  return monecos.sellCount > 15;
});
console.log("🚀 ~ losMasVendidos ~ losMasVendidos:", losMasVendidos);

losMasVendidos.forEach((monecos) => {
  console.log(`Los monecos más vendidos son: ${monecos.name}`);
});
